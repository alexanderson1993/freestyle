...
const TableHeader = <T extends object>({
  ...
  <AriaTableHeader
    className={composeRenderProps(className, (className) =>
      cn("[&_tr]:border-b [&_tr]:border-b-muted", className)
    )}
    {...props}
  />
);
    ...
    {composeRenderProps(children, (children, { allowsSorting }) => (
      <div className="flex items-center">
        ...
        >
          ...
          {allowsSorting && <Icon name="ArrowUpDown" className="ml-2 size-4" />}
        </Group>
        {props.isResizable && (
...
const Row = <T extends object>({ className, ...props }: RowProps<T>) => (
  <AriaRow
    className={composeRenderProps(className, (className) =>
      cn(
        "border-b border-input -outline-offset-2 transition-colors data-[hovered]:bg-muted/50 data-[selected]:bg-muted data-[focus-visible]:outline-ring",
        className
      )
    )}
    {...props}
